...
const chatId = globalThis.TG_BOT_CHATID;
...
const getCookieConfig = async () => {
  ...
  if (!MYSCookies) {
    ...
    bot.sendMessage(chatId, "Missing required environment variables: MYS_COOKIES.");
...
const getRole = async (cookie, gameKey) => {
  ...
  const res = await $axios.request({
    ...
    bot.sendMessage(chatId, `Login error for ${gameKey}:\n` + err);
  })
  if (res.data['retcode'] !== 0) {
    console.info('Account not logged in, please check cookie', JSON.stringify(res.data))
    bot.sendMessage(chatId, `Account not logged in for ${gameKey}, please check cookie:\n` + JSON.stringify(res.data));
  }
  if ((res?.data?.message === 'OK') && res.data.data.list[0]) {
    ...
    bot.sendMessage(chatId, `[${gameKey}] Login successful <${ROLE[gameKey].nickname}(${ROLE[gameKey].game_uid})>`);
  } else {
    ROLE[gameKey] = {
      game_biz: '',
      region: '',
    ...
    bot.sendMessage(chatId, `[${gameKey}] Login failed <No character found>:\n` + JSON.stringify(res.data));
  }
}
...
async function Sign_In(cookie, gameKey) {
  ...
  const res = await $axios.request({
    ...
    bot.sendMessage(chatId, `Sign-in error for ${gameKey} <${ROLE[gameKey].nickname}(${ROLE[gameKey].game_uid})>:\n` + err);
  })
    ...
    bot.sendMessage(chatId, `<${ROLE[gameKey].nickname}(${ROLE[gameKey].game_uid})> Sign-in ${res?.data?.message === 'OK' ? 'successful' : 'failed'}:\n` + JSON.stringify(res.data));
  }
...
const doMYSSign = async (gameKey) => {
  ...
  if (cookieList.length) {
    ...
    bot.sendMessage(chatId, `[${gameKey}] Start signing in, total ${cookieList.length} users`);
    for (const cookIndex in cookieList) {
      ...
      if (cook) {
        ...
        bot.sendMessage(chatId, `[${gameKey}] User ${Number(cookIndex) + 1} starts signing in...`);
    ...
    bot.sendMessage(chatId, `[${gameKey}] Sign-in completed`);
  }
}
